...
function afterHeatmap(paisesObj, opts) {
  opts = opts || {};
  const showValor = opts.showValor !== false;
  const P = paisesObj || {por_pais: [], registros: []};
  const tblEl = document.getElementById('tblPaises');
  if (tblEl) {
    tblEl.innerHTML = `
      <tr><th>País</th><th>Registros</th><th>Origem</th>${opts.showIesDestino ? '<th>IES Destino</th>' : ''}${showValor ? '<th>Valor</th>' : ''}</tr>
      ${P.por_pais.map(p => {
        let iesCol = '';
        ...
        return `<tr><td>${p.pais}</td><td class="num">${p.total_registros}</td>
          <td>${Object.entries(p.por_origem).map(([k,v])=>`${k} (${v})`).join(', ')}</td>${iesCol}${showValor ? `<td class="num">${fmtBRL(p.valor_total)}</td>` : ''}</tr>`;
      }).join('')}`;
  }
...
const ICONS = {
  ...
  parcerias: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M11 7.5L7.8 10.6c-.6.6-.6 1.6 0 2.2.6.6 1.6.6 2.2 0l2.6-2.5 1.1 1-2.7 2.6c-1.2 1.1-3 1.1-4.2 0-1.1-1.1-1.1-2.9 0-4.1L10.4 6c.9-.9 2.4-.9 3.3 0l3.6 3.5c1 .9 1 2.5 0 3.4l-3.2 3.1-1-1.1 3.1-3c.4-.4.4-1 0-1.4l-3.6-3.5c-.3-.3-.8-.3-1.1 0z"/><circle cx="5" cy="19" r="2"/><circle cx="19" cy="19" r="2"/><path d="M7 18.3l5-3.3 5 3.3" fill="none" stroke="currentColor" stroke-width="1.4"/></svg>',
};
...
const NAV = [
  ...
  { id:'parcerias', label:'Parcerias Prioritárias', icon:ICONS.parcerias },
...
const TITLES = { quemsomos:['Sobre a Rede','Rede CAPES Global para o Desenvolvimento Sustentável, Ciência e Saúde.'],
  ...
  parcerias:['Parcerias Prioritárias','Instituições parceiras por tema estratégico, região e país.'],
};
...
function buildNav() {
  ...
  NAV.forEach(item => {
    if (item.children) {
      ...
      head.onclick = () => { head.classList.toggle('expanded'); sub.classList.toggle('open'); syncNavRail(); setTimeout(syncNavRail, 200); };
...
function syncNavRail() {
  // Mede a posição real do primeiro e do último ponto (itens de nível 0) e redimensiona
  // o trilho pra conectar exatamente do primeiro ao último, não importa quantos itens existam.
  const nav = document.getElementById('navRoot');
  const rail = document.getElementById('navRail');
  const dots = [...nav.querySelectorAll('.nav-item.lvl0')];
  if (!rail || dots.length < 2) return;
  const navBox = nav.getBoundingClientRect();
  const firstBox = dots[0].getBoundingClientRect();
  const lastBox = dots[dots.length - 1].getBoundingClientRect();
  const firstCenter = (firstBox.top + firstBox.height / 2) - navBox.top;
  const lastCenter = (lastBox.top + lastBox.height / 2) - navBox.top;
...
function render() {
  ...
  try {
    ...
    else if (route === 'parcerias') { setTitle(...TITLES.parcerias); root.innerHTML = pageParcerias(); afterParcerias(); }
...
// ============================================================
// PAGE: Parcerias Prioritárias
// ============================================================
function pageParcerias() {
  const pp = DATA.parcerias_prioritarias;
  if (!pp) {
    return `<div class="panel"><div class="empty-state">Ainda não há dados de Parcerias Prioritárias carregados neste painel.</div></div>`;
  }
  const temas = pp.resumo_por_tema.map(t => t.tema);
  const paisesUnicos = [...new Set(pp.registros.map(r => r.pais).filter(Boolean))].sort();
  return `
    <div class="kpi-row">
      ${kpiCard('Total de parcerias', fmtNum(pp.resumo_total.total))}
      ${kpiCard('Sul Global', fmtNum(pp.resumo_total.sul))}
      ${kpiCard('Norte Global', fmtNum(pp.resumo_total.norte))}
      ${kpiCard('Temas estratégicos', fmtNum(pp.resumo_por_tema.length))}
    </div>

    <div class="section-title">Resumo por tema</div>
    <div class="grid-2">
      <div class="panel"><h3>Parcerias Sul x Norte Global por tema</h3><div class="chart-box"><canvas id="chParceriasTema"></canvas></div></div>
      <div class="panel"><h3>Detalhamento</h3><table>
        <tr><th>Tema</th><th>Sul Global</th><th>Norte Global</th><th>Total</th></tr>
        ${pp.resumo_por_tema.map(t => `<tr><td>${t.tema}</td><td class="num">${fmtNum(t.sul)}</td><td class="num">${fmtNum(t.norte)}</td><td class="num">${fmtNum(t.total)}</td></tr>`).join('')}
        <tr style="font-weight:600;"><td>TOTAL</td><td class="num">${fmtNum(pp.resumo_total.sul)}</td><td class="num">${fmtNum(pp.resumo_total.norte)}</td><td class="num">${fmtNum(pp.resumo_total.total)}</td></tr>
      </table></div>
    </div>

    <div class="section-title">Presença internacional das parcerias</div>
    ${heatmapBlock(pp, {title:'Mapa de calor — países parceiros'})}

    <div class="section-title">Lista de parcerias <span class="tag" id="ppFilterCount">${fmtNum(pp.registros.length)} registros</span></div>
    <div class="panel">
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:16px;">
        <div>
          <label style="font-size:10.5px;text-transform:uppercase;color:var(--muted);font-weight:600;display:block;margin-bottom:4px;">Tema</label>
          <select id="ppFiltroTema" onchange="applyParceriasFilter()" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:12.5px;background:#fff;">
            <option value="">Todos os temas</option>
            ${temas.map(t => `<option value="${t}">${t}</option>`).join('')}
          </select>
        </div>
        <div>
          <label style="font-size:10.5px;text-transform:uppercase;color:var(--muted);font-weight:600;display:block;margin-bottom:4px;">Região</label>
          <select id="ppFiltroRegiao" onchange="applyParceriasFilter()" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:12.5px;background:#fff;">
            <option value="">Todas</option>
            <option value="Sul Global">Sul Global</option>
            <option value="Norte Global">Norte Global</option>
          </select>
        </div>
        <div>
          <label style="font-size:10.5px;text-transform:uppercase;color:var(--muted);font-weight:600;display:block;margin-bottom:4px;">País</label>
          <select id="ppFiltroPais" onchange="applyParceriasFilter()" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:12.5px;background:#fff;">
            <option value="">Todos os países</option>
            ${paisesUnicos.map(p => `<option value="${p}">${p}</option>`).join('')}
          </select>
        </div>
        <div>
          <label style="font-size:10.5px;text-transform:uppercase;color:var(--muted);font-weight:600;display:block;margin-bottom:4px;">Instituição</label>
          <input id="ppFiltroInst" type="text" oninput="applyParceriasFilter()" placeholder="Buscar por nome..." style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:12.5px;box-sizing:border-box;">
        </div>
      </div>
      <div style="max-height:480px;overflow-y:auto;">
        <table id="tblParcerias"></table>
      </div>
    </div>
  `;
}
function afterParcerias() {
  const pp = DATA.parcerias_prioritarias;
  if (!pp) return;
  safeChart('chParceriasTema', { type:'bar', data:{ labels: pp.resumo_por_tema.map(t=>t.tema),
    datasets:[{label:'Sul Global', data:pp.resumo_por_tema.map(t=>t.sul), backgroundColor:'#2563a8', borderRadius:4},
               {label:'Norte Global', data:pp.resumo_por_tema.map(t=>t.norte), backgroundColor:'#c98a2a', borderRadius:4}]},
    options: countBarOpts() });
  afterHeatmap(pp, {showValor: false});
  renderParceriasTable(pp.registros);
}
function renderParceriasTable(registros) {
  const tbl = document.getElementById('tblParcerias');
  if (!tbl) return;
  tbl.innerHTML = `
    <tr><th>Tema</th><th>Região</th><th>Instituição Parceira</th><th>País</th></tr>
    ${registros.map(r => `<tr><td>${r.tema||'—'}</td><td>${r.regiao||'—'}</td><td>${r.instituicao||'—'}</td><td>${r.pais||'—'}</td></tr>`).join('')}
    ${registros.length === 0 ? '<tr><td colspan="4" style="text-align:center;color:var(--muted);padding:20px;">Nenhuma parceria encontrada com esses filtros.</td></tr>' : ''}
  `;
}
function applyParceriasFilter() {
  const pp = DATA.parcerias_prioritarias;
  if (!pp) return;
  const tema = document.getElementById('ppFiltroTema').value;
  const regiao = document.getElementById('ppFiltroRegiao').value;
  const pais = document.getElementById('ppFiltroPais').value;
  const busca = document.getElementById('ppFiltroInst').value.trim().toLowerCase();
  const filtrados = pp.registros.filter(r =>
    (!tema || r.tema === tema) &&
    (!regiao || r.regiao === regiao) &&
    (!pais || r.pais === pais) &&
    (!busca || (r.instituicao||'').toLowerCase().includes(busca))
  );
  renderParceriasTable(filtrados);
  const countEl = document.getElementById('ppFilterCount');
  if (countEl) countEl.textContent = fmtNum(filtrados.length) + ' registros';
}
